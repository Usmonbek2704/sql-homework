...
select c.FirstName, o.OrderDate,   o.OrderID
from Customers c
cross apply (
          select OrderDate, OrderID
		  from Orders o
		  where year(orderdate)>2022 and c.CustomerID=o.CustomerID)
		  as o
...
from Employees e
   cross apply (
		  ...
		  from Departments d
		  where d.DepartmentID=e.DepartmentID and (d.DepartmentName='sales' or  d.DepartmentName='marketing'))
		  as d
--Return: DepartmentName, MaxSalary
--Task: Show the highest salary for each department.
--Tables Used: Departments, Employees
select e.maxsalary, d.DepartmentName
from Departments d
cross apply (
...
select c.FirstName, a.OrderID, a.OrderDate
from Customers c
 cross apply(
           select o.OrderDate, o.OrderID
		   from Orders o
		   where c.CustomerID=o.CustomerID and c.Country='usa' and o.OrderDate='2023')
		   as a
--Return: CustomerName, TotalOrders
--Task: Show how many orders each customer has placed.
--Tables Used: Orders , Customers
select c.FirstName, a.totalorders
from Customers c
cross apply (
...
select a.SupplierName, p.ProductName
from Products p
cross apply(
          select s.SupplierName
		  from Suppliers s
		  where s.SupplierID=p.SupplierID and s.SupplierName in ('gadget supplies', 'clothing mart'))
		  as a
...
--Tables Used: Customers, Orders
select c.FirstName, a.OrderDate
from Customers c
cross apply(
...
--Tables Used: Orders, Customers
select a.TotalAmount, c.FirstName
from Customers c
outer join (
          select O.TotalAmount
		  from  Orders o
		  where c.CustomerID=o.CustomerID and  o.TotalAmount>500)
		  as a
--Return: ProductName, SaleDate, SaleAmount
--Task: List product sales where the sale was made in 2022 or the sale amount exceeded 400.
--Tables Used: Products, Sales
select a.SaleDate, a.SaleAmount, i.ProductName
from Products i
cross apply (
          select s.SaleDate, s.SaleAmount
		  from Sales s
		  where s.ProductID=i.ProductID  and year(s.saledate)=2022 and s.SaleAmount>400)
		  as a

--Return: ProductName, TotalSalesAmount
--Task: Display each product along with the total amount it has been sold for.
--Tables Used: Sales, Products
select p.ProductName, a.abc as TotalSalesAmount
from Products p  
outer apply (
    select  SUM(s.SaleAmount) as  abc
	from   Sales s
...
--Tables Used: Employees, Departments
select a.DepartmentName, e.Name, e.Salary
from Employees e
cross apply (
         select d.DepartmentName
		 from Departments d
		 where d.DepartmentID=e.DepartmentID
		and d.DepartmentName='human resources' and e.Salary>6000 )
		 as a
...
--Tables Used: Products, Sales
select p.ProductName, a.SaleDate, p.StockQuantity
from Products p
cross apply (
           select s.SaleDate
		   from Sales s
		   where p.ProductID=s.ProductID and year(s.saledate)=2023 and p.StockQuantity>100)
		   as a
...
--Tables Used: Employees, Departments
select a.DepartmentName, e.Name, e.HireDate
from Employees e
cross apply (
          select d.DepartmentName
		  from Departments d
		  where d.DepartmentID=e.DepartmentID and d.DepartmentName='sales' and year(e.HireDate)<2020)
		  as a
--🔴 Hard-Level Tasks (7)
--Return: CustomerName, OrderID, Address, OrderDate
--Task: List all orders made by customers in the USA whose address starts with 4 digits.
--Tables Used: Customers, Orders
select c.FirstName, a.OrderID, c.Address, a.OrderDate
from Customers c
cross apply(
          select o.OrderID, o.OrderDate
		  from Orders o
		  where c.CustomerID=o.CustomerID and c.Country='usa' and c.Address like '[0-9][0-9][0-9][0-9]%')
		  as a
...
--Tables Used: Products, Sales
select a.SaleAmount, p.Category, p.ProductName
from Products p
cross apply (
          select s.SaleAmount
		  from Sales s
		  where p.ProductID=s.ProductID and (p.Category='electronics' or s.SaleAmount>350))
		  as a
...
select a.productcount, c.CategoryName
from  Categories c
cross apply (
             select    sum(p.StockQuantity) as productcount
			 from Products p
...
--Tables Used: Customers, Orders
select c.FirstName, c.City, a.OrderID, a.TotalAmount
from Customers c
cross apply(
       select o.OrderID, o.TotalAmount
	   from Orders o
	   where c.CustomerID=o.CustomerID and c.City='los angeles' and o.TotalAmount>300)
	   as a
--Return: EmployeeName, DepartmentName
...
--Tables Used: Employees, Departments
select e.Name, a.DepartmentName, e.Salary
from Employees e
cross apply (
          select d.DepartmentName
		  from Departments d
		  where d.DepartmentID=e.DepartmentID and d.DepartmentName in ('sales', 'marketing') and e.Salary>60000) as a
