...
--Display the first name and the length of the first name for all employees whose name starts with the letters 'A', 'J' or 'M'. Give each column an appropriate label.
--Sort the results by the employees' first names.(Employees)'
select first_name, len(first_name) as lent from Employees
...
select GREATEST(max1, max2, Max3), Year1 from TestMax
...
select  coalesce(ssn, passportid, itin ) as firstnonvalue  from person
--Medium Tasks
...
select count(*) as counts,  100*count(FIRST_NAME)/ count(*) as percentage from Employees
where GETDATE()-year(HIRE_DATE)>3
