...
select 
     substring (vals, 1, charindex('.' , vals) -1) as hgth,
	 substring(vals, charindex('.' , vals)+1, len(vals)) as buihut
from splitter
...
select 
len(texts)-len(replace(texts, ' ', '') )as counrts4
from CountSpaces
--Write a SQL query that finds out employees who earn more than their managers.(Employee)
select e2.Name, e2.Salary, e1.Name, e1.Salary
from Employee e2
inner join Employee e1 on e1.Id=e2.ManagerId
and e1.Salary<e2.Salary
--Find the employees who have been with the company for more than 10 years, but less than 15 years. Display their Employee ID, First Name, Last Name, Hire Date, 
...
select w1.Id, w2.RecordDate, w2.Temperature  from weather w1
join weather w2 
...
select player_id, device_id, event_date, games_played, dateadd(day, -games_played, event_date) as logindaye from Activity
...
select id , left(vals, 1) from GetIntegers
where left(vals,1) like '%[0-9]%'
...
PARSENAME(REPLACE(vals, ',', '.'),1) as swappedvaslsa
from MultipleVals
...
select left('rtcfvty34redt', 7) as hg,
right('rtcfvty34redt', 4) as ngtgoi,
substring('rtcfvty34redt',8, 2 ) as kgnnhgio
