...
create procedure for_employees2
as 
begin 
     ...
     select e.EmployeeID,  concat(e.firstname, ' ', e.LastName) as fullname, e.Department,e.Salary, (e.Salary*d.BonusPercentage/100) as bounusamount
	 from Employees e
	 join DepartmentBonus d on d.Department=e.Department
...
create procedure for_salary 
as 
begin
     select d.Department, ((100+d.BonusPercentage)*e.Salary)/100 as bonussalary
	 from Employees e
	 join DepartmentBonus d on d.Department=e.Department
end;
exec for_salary
...
merge into products_new pn
using Products_Current pc
on pn.productid=pc.productid
when matched then 
     update set pn.ProductName=pc.ProductName, pn.Price=pc.Price
when not matched by target then
     insert (productid, ProductName, Price)
	 values (pc.productid, pc.ProductName, pc.Price)
...
SELECT 
    ...
    t.id,
    CASE 
        WHEN c.id IS NULL THEN 'Root'
        WHEN c.child_id IS NULL THEN 'Leaf'
        ELSE 'Inner'
    END AS node_type
FROM tree t
LEFT JOIN (
    SELECT id, COUNT(*) AS child_id
    FROM tree
...
select *,
case 
    when action='confirmed' and sumaction=1 then 0.5
	when action='confirmed' and sumaction>1 then 1
	when action='timeout' then 0
	else 0
	end as level
from (
select c.user_id, c.action , count(c.action)  as sumaction 
from Confirmations c
 join Signups s on c.user_id=s.user_id
group by c.action, c.user_id) as countconfirmations
...
CREATE PROCEDURE  for_products
as
begin
     select p.productname, sum(s.Quantity) as Total_Quantity_Sold,
	 (s.Quantity × p.Price) as Total_Sales_Amount,
	 max(s.saledate) as First_Sale_Date,
	 min(s,saledate) as last_sale_date 
	 coalesce(
     from Products p
