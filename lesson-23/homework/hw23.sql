...
select *, 
RIGHT('0' +cast(month(dt) as varchar(2)), 2) as bbrutbbvt
from Dates
...
select distinct_ids, rids, sum(maxvals) over (partition by rids)   as totalmaxvals
from (
select count(distinct id)   as distinct_ids, 
max(rid) over (order by rid) as rids,
max(vals) over (partition by id) as maxvals
from MyTabel
group by Id)
as nguthgtti
...
select * from (
select *, len(vals) as lenth
from TestFixLengths) as nvntg5gt
where lenth between 6 and 10

--Puzzle 4: In this puzzle you have to find the maximum value for each 
--Id and then getthe Item for that Id and Maximum value. Please check out sample input and expected output.
...
from TestMaximum) as bgvhbgvtnb
where rn=1

--Puzzle 5: In this puzzle you have to first find the maximum value for each
--Id and DetailedNumber, and then Sum the data using Id only. Please check out sample input and expected output.
...
select id, sum(vrc) as nvujut	
from (
select id, max(vals)  as vrc
from SumOfMax
group by detailednumber, Id) as nvuyt
group by Id
...
from
(select *, (a-b) as outputs
from TheZeroPuzzle) AS fth
...
select sum(QuantitySold*UnitPrice)/SUM(QuantitySold) as avarageunitprice
from Sales
--How many sales transactions were recorded?
select count(QuantitySold)
from Sales
--What is the highest number of units sold in a single transaction?
select top 1 (UnitPrice*QuantitySold) as highesttransactiom
from Sales
--How many products were sold in each category?
select *, sum(QuantitySold) over (partition by category) as gyfyy
from Sales
--What is the total revenue for each region?
select *, sum(QuantitySold*UnitPrice) over (partition by region) as jgfhf
from Sales
--Which product generated the highest total revenue?
select top 1 (UnitPrice*QuantitySold) as highesttransactiom
from Sales
--Compute the running total of revenue ordered by sale date.
select *, sum(UnitPrice*QuantitySold) over (
order by saledate
rows between unbounded preceding and current row)
from Sales
--How much does each category contribute to total sales revenue?
select *, sum(UnitPrice*QuantitySold) over (partition by category) as revenueforcategory
from Sales


--Show all sales along with the corresponding customer names
select c.CustomerID, c.CustomerName, s.Product, s.QuantitySold,s.UnitPrice
from Customers c
join Sales s on s.CustomerID=c.CustomerID
order by c.CustomerID
--List customers who have not made any purchases
select c.CustomerID, c.CustomerName, s.Product, s.QuantitySold,s.UnitPrice, s.SaleID
from Customers c
left join Sales s on c.CustomerID=s.CustomerID
where s.SaleID is null
--Compute total revenue generated from each customer
select c.CustomerID, c.CustomerName, s.Product,
s.QuantitySold,
s.UnitPrice,
s.SaleID, 
(s.QuantitySold*s.UnitPrice) as profit
from Customers c
left join Sales s on c.CustomerID=s.CustomerID
...
(s.QuantitySold*s.UnitPrice) as profit
from Customers c
left join Sales s on c.CustomerID=s.CustomerID) as jfnuygy
where profit=(select max(QuantitySold*UnitPrice) from Sales)
...
select *
from (
select *, avg(sellingprice) over (partition by category) as avarageprice
from Products) as jfnbu
where avarageprice<SellingPrice
