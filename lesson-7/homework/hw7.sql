...
select max(salary) as maxs from Employees
select count(*) as countrow from Customers
select count(distinct productname) from Products
select saleid, sum(saleamount) as salea from Sales
group by SaleID, ProductID
having ProductID=7

select avg(age) as avgage from Employees
select DepartmentName, count(EmployeeID) as counte from Employees
group by DepartmentName
select category, min(price) as minp, max(price) as maxp from Products
group by Category
select customerid, sum(saleamount) as salea from Sales
group by CustomerID
select DepartmentName from Employees
group by DepartmentName
having count(DepartmentName)>5
select sum(SaleAmount) as sums, avg(Saleamount) as avgs from Sales
group by ProductID
select count(departmentname) as counts from Employees
group by DepartmentName
...
select min(salary) as mins, max(salary) as maxs from Employees
group by DepartmentName
...
select avg(salary), count(employeeid) from Employees
group by DepartmentName
...
select year(saledate)as year, sum(saleamount) from Sales
group by SaleDate
order by year 
--Write a query to show the list of customers who placed at least 3 orders.
SELECT FirstName, COUNT(Country) AS counts
FROM Customers
GROUP BY FirstName
...
select departmentname, avg(salary) as avgs from Employees
group by DepartmentName
...
select productname, avg(price) as avgp from Products
...
select sum(TotalAmount) as sums, customerid  from Orders
group by CustomerID
...
select departmentname, sum(salary) as sums,  avg(salary) from Employees
group by DepartmentName
...
GROUP BY    custid
HAVING SUM(CASE WHEN Freight > 50 THEN Freight ELSE 0 END) > 0;
...
select MONTH(orderdate)as monthp,
sum(quantity) as sumq,
sum(distinct quantity) as sumdq
from Orders
group by month(orderdate)
having sum(distinct quantity)>2
--Write a query to find the MIN and MAX order quantity per Year. From orders table.
select year(orderdate) as date, min(quantity) as minq, max(quantity) as maxq from Orders
