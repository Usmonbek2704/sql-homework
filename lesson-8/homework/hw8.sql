...
select  count(StockQuantity), category from Products
group by Category


--Using Products table, get the average price of products in the 'Electronics' category.
select avg(StockQuantity) as avgs from Products
group by category
...
select top 1 ProductName, StockQuantity from Products
...
select count(customerid) as countid, country from Customers
group by country
--Using Orders table, find the minimum and maximum quantity ordered.
select max(quantity) as maxq, min(quantity) AS minq from Orders
...
select avg(quantity) as avgq, year(orderdate) from Orders
...
select  sum(saleamount) as summfb, productid from Sales
group by productid
...
select top 3 totalamount, CustomerID from Invoices
