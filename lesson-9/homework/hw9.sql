...
select p.ProductName, o.OrderID, o.CustomerID, o.OrderDate,o.Quantity, o.TotalAmount
from Products p
...
select d.DepartmentName, e.Name, e.DepartmentID, e.Salary, e.HireDate, e.ManagerID
from Employees e
inner join Departments d on d.DepartmentID=e.DepartmentID
--Using Students, Enrollments table List student names and their enrolled course IDs.
select s.Name, s.Age, s.Major, e.CourseID
from Students s
...
SELECT o.OrderID, o.CustomerID, o.OrderDate, p.PaymentID, p.Amount
FROM Orders o
INNER JOIN Payments p 
    ON o.OrderID = p.OrderID;

--Using Orders, Products table Show orders where product price is more than 100.
select p.ProductName, p.Price, p.Category, p.StockQuantity, p.SupplierID, o.OrderID
from Products p
inner join Orders o on o.ProductID=p.ProductID
where p.Price>100
--🟡 Medium (10 puzzles)
...
select c.FirstName, c.LastName, o.TotalAmount, c.Country
from Orders o
...
select p.Amount, pr.Price, o.Quantity
from Payments p
...
select s.Name, c.CourseName
from Students s
full outer join Enrollments e on e.StudentID=s.StudentID
...
select e.Name, em.Name, e.Salary
from Employees e
